...
public class AdministrationModule {
    ...
    public void administrationOperation() throws DoctorNotFoundException, StaffNotFoundException, PatientNotFoundException {
    ...
    public void doViewAllPatients() {
        ...
        for (PatientEntity patientEntity : patientEntities) {
            System.out.printf("%8s%20s%20s%20s%20s%20s%20s%20s%20s\n", patientEntity.getPatientId().toString(), patientEntity.getIdentityNumber(), patientEntity.getFirstName(), patientEntity.getLastName(), patientEntity.getGender(), patientEntity.getAge(), patientEntity.getPhone(), patientEntity.getAddress(), patientEntity.getSecurityCode().toString());
        }
    ...
    public void doDeletePatient() throws PatientNotFoundException {
        ...
        System.out.printf("Confirm Delete Patient %s %s (Patient Identity Number: %s) (Enter 'Y' to Delete)> ", patientEntity.getFirstName(), patientEntity.getLastName(), patientEntity.getIdentityNumber());
        input = scanner.nextLine().trim();
